...
class ChaseURSapphireSpider(CrawlSpider):
    store_name = "Chase UR (Sapphire)"
    name = "chaseursapphire"
    ...
    def __init__(self, *args, **kwargs):
        super(ChaseURSapphireSpider, self).__init__(*args, **kwargs)
    ...
    def parse_product(self, response):
        ...
        for td in tr:
            if len(td.xpath('td[7]/div/a/text()')):
                name        = str(td.xpath('td[1]/a/text()').extract_first().encode('utf-8'))
                link        = str(td.xpath('td[7]/div/a/@href').extract_first())
                cashback    = str(td.xpath('td[7]/div/a/text()').extract_first().encode('utf-8'))
                ...
                yield item
            elif len(td.xpath('td[7]/a/text()')):
                name        = str(td.xpath('td[1]/a/text()').extract_first().encode('utf-8'))
                link        = str(td.xpath('td[7]/a/@href').extract_first())
                cashback    = str(td.xpath('td[7]/a/text()').extract_first().encode('utf-8'))
                item['name']        = name.replace("\r\n", "").replace("'", "''")
                item['link']        = self.base_url + link
                item['cashback']    = cashback.replace("'", "''")
                item['sid']         = self.store_name
